...
**Lab Architect**
    Traffic flows through the environment in the following order:
        ...
        The attacker machine (Kali Linux at 192.168.0.203) sends requests to the domain name myweb.testing.

        The domain is mapped using the local hosts file on both Kali and Ubuntu to the IP address 192.168.0.28.

        All requests reach the Safeline WAF, which sits in front of the web server as a reverse proxy.

        Safeline analyzes each request. If the request is malicious, it blocks it and logs the attack. If the request is clean, Safeline forwards it to the Apache server on Ubuntu.

        The Ubuntu server (192.168.0.28) runs DVWA and responds to the request.

        The response passes back through the WAF and returns to Kali.
...
**Testing and Results**
    Using Kali, I attempted multiple categories of attacks against DVWA through the domain myweb.testing. Safeline WAF detected and blocked:
